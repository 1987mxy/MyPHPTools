...
class db_mysql
{
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname = '', $pconnect = 0, $charset = '')
	{
		...
		{
			if(TF_MYSQL_DB == '' && file_exists(PHPCMS_ROOT.'install.php'))
			{
	...
	function query($sql , $type = '')
	{
		...
		function insert_c($dbname, $array)
	{
		
		$this->check_fields($dbname, $array);
		return $this->query("INSERT INTO `$dbname`(`".implode('`,`', array_keys($array))."`) VALUES('".implode("','", $array)."')");
	}
